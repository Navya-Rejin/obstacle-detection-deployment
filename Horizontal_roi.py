...
def define_polygonal_roi(frame, shape="trapezium"):
    ...
    if shape == "trapezium":
        # Define vertices for a trapezoidal ROI
        vertices = np.array([[
            (80, height),               # Bottom-left corner
            (width - 80, height),       # Bottom-right corner
            (width // 2 + 50, height // 3 + 120),  # Top-right corner (narrower)
            (width // 2 - 50, height // 3 + 120)   # Top-left corner (narrower)
        ]], dtype=np.int32)
...
while True:
    ...
    for i, r in enumerate(results):
        for box in r.boxes:
            ...
            # Logic to determine if the object is inside the ROI
            ...
            if color == (0, 0, 255):  # Red box indicates inside the ROI
                ...
                print("Object outside the blue bounding box. Slow down the trolley.")
